...
#         self.toolbar.AddSimpleTool(wx.NewId(), images.getNewBitmap(), 'New', '''Long help for New''')
        ...
        self.bizLabel = wx.StaticText(self.panel, -1, u'业务号', pos=tuple(self.bizLabelPos))

        self.bizNoPos = [self.bizLabelPos[0]+self.bizLabel.GetSize()[0]+self.DefaultInterval, self.bizLabelPos[1]]
        self.bizNoSize = [200,25]
        self.bizNo = wx.TextCtrl(self.panel, -1, value = u'2009051911394580475', 
        ...
        self.bizRBType.SetStringSelection(u'赔案')
        #赔案版本复选框
...
#         self.ScreenRButton.SetPosition(tuple(self.ScreenRButtonPos))
    ...
    def OnSearchClick(self, event):
        if self.bizNo.Value == '':
            bizDlg = wx.MessageDialog(None, u"业务号不能为空", u"ERROR", wx.YES_DEFAULT|wx.ICON_QUESTION)
            if(bizDlg.ShowModal() == wx.ID_YES):
                pass
            bizDlg.Destroy()
        else:
            ...
            data = self.getChildren(self.bizNo.Value, self.CateSelected, self.TypeSelected)
            if(data == None) :
                bizDlg = wx.MessageDialog(None, u"未查询到结果", u"ERROR", wx.YES_DEFAULT|wx.ICON_QUESTION)
                if(bizDlg.ShowModal() == wx.ID_YES):
                    bizDlg.Destroy()
                    return
                
            self.AddTreeNodes(self.bizRoot, data)
            
#             for Child in self.ChildSet:
#                 self.bizTree.AppendItem(self.bizRoot, str(Child[0]))
            

            self.Bind(wx.EVT_TREE_ITEM_EXPANDED, self.OnItemExpanded, self.bizTree)
            self.Bind(wx.EVT_TREE_ITEM_COLLAPSED, self.OnItemCollapsed, self.bizTree)
            self.Bind(wx.EVT_TREE_SEL_CHANGED, self.OnSelChanged, self.bizTree)
            self.Bind(wx.EVT_TREE_ITEM_ACTIVATED, self.OnActivated, self.bizTree)
            self.Bind(wx.EVT_TREE_ITEM_MENU, self.OnActivated, self.bizTree)
            
            self.bizTree.Expand(self.bizRoot)
            self.MySetTextViewDisable()
