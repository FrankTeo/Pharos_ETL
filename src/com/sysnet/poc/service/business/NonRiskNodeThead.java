...
public class NonRiskNodeThead extends Thread {
	...
	public NonRiskNodeThead(List<InsideProcess> a) {
		super();
		j.setName("非风险");
		a.add(j);
	}
	...
	public void run() {
		try {
			if("1".equalsIgnoreCase(dealMode)){
				//20140827 Frank Zhang FOR Remove beyond Insert
...
//			log.info("循环等待的时间：" + (e - f));
			Long riskNodeid = nonRiskNodeIDlist.get(temp);
			// BContractNode node = bnMap.get(riskNodeid);
//			Long a = System.currentTimeMillis();
			BContractNode node = bContractBo.getSingleNode(riskNodeid);
			if (node.getNodeNo().equals("1")) {
				node = bContractBo.getContractNode(riskNodeid);
			}
//			Long b = System.currentTimeMillis();
//			log.info("取节点的时间：" + (b - a));
			//log.info("PolicyNo=" + node.getPolicyNo() + " " + "ContractNo=" + node.getContractNo() + " " + "ObjectId=" + this.object_Id + " " + ",node size=" + nonRiskNodeIDlist.size() + ",now:" + temp);
			String dcsNodeId = node.getDcsNodeCode();
//			Long b1 = System.currentTimeMillis();
//			log.info("取节点模板的时间1：" + (b1 - b));
			ProductMappingBuilder singleProductMappingBuilder = new ProductMappingBuilder(PRODUCT_CD, node, "1", bContractBo, null);
...
//			log.info("取节点模板的时间3：" + (c - b2));
			...
			globalDataMap.put("endorseno", node.getPolicyNo());
			...
			//globalDataMap.put("mainversion", node.getMainVersion() + "");	//delete by zhangfan 20140310
