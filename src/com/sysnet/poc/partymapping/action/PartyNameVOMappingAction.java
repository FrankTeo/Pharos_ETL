...
/**
 * STG_PTY_PARTY_NAME--当事方_名称信息映射处理
 * 
 * @author lu_haibin
 * 
 */

public class PartyNameVOMappingAction extends AbsPharosMappingAction {
	...
	public boolean execute() {
		...
		try {
			...
			// 如果是引用取引用的值
				...
				PartyNameVO partyNameVO = (PartyNameVO) this.provideInstnace;
				String methodNm = item.getMethod();
				Object obj = null;
				// 调用反射
				if (methodNm == null) {
					log.error("class PartyNameVOMappingAction report:" + item.getParameter() + "get method name is null!");
				} else {
					if (methodNm.indexOf(".") != -1) {
						String temp[] = methodNm.split("[.]");
						for (int i = 0; i < temp.length; i++) {
							if (i == 0) {
								obj = ReflectUtil.getObjByReflect(partyNameVO, temp[i]);
							} else {
						...
						obj = ReflectUtil.getObjByReflect(partyNameVO, methodNm);
					}
				}
			...
			log.error("PartyNameVOMappingAction:", ex);
			ex.printStackTrace();
		}
		return rs;
