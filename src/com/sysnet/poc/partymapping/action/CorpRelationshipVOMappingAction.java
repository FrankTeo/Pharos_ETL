...
/**
 * STG_PTY_CORP_RELATIONSHIP--当事方_公司关系表信息映射处理
 * 
 * @author lu_haibin
 * 
 */

public class CorpRelationshipVOMappingAction extends AbsPharosMappingAction {
	...
	public boolean execute() {
		...
		try {
			...
			// 如果是引用取引用的值
				...
				CorpRelationshipVO corpRelationshipVO = (CorpRelationshipVO) this.provideInstnace;
				String methodNm = item.getMethod();
				Object obj = null;
				// 调用反射
				if (methodNm == null) {
					log.error("class CorpRelationshipVO report:" + item.getParameter() + "get method name is null!");
				} else {
					if (methodNm.indexOf(".") != -1) {
						String temp[] = methodNm.split("[.]");
						for (int i = 0; i < temp.length; i++) {
							if (i == 0) {
								obj = ReflectUtil.getObjByReflect(corpRelationshipVO, temp[i]);
							} else {
						...
						obj = ReflectUtil.getObjByReflect(corpRelationshipVO, methodNm);
					}
				}
			...
			log.error("CorpRelationshipVOMappingAction:", ex);
			ex.printStackTrace();
		}
		return rs;
